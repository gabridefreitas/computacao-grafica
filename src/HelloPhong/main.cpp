 ...
 * https://learnopengl.com/#!Getting-started/Hello-Triangle e
 * https://antongerdelan.net/opengl/ Adaptado por Rossana Baptista Queiroz para
 * as disciplinas de Processamento Gráfico/Computação Gráfica - Unisinos Versão
 * inicial: 7/4/2017 Última atualização em 03/03/2026
 */
...
int main() {
  ...
  GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Olá Camera Sintética!",
                                        nullptr, nullptr);
  ...
  glUniform3f(glGetUniformLocation(shaderID, "lightColor"), lightColor.x,
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS) {
      ...
      view = glm::lookAt(glm::vec3(-3, 0, 0), glm::vec3(0, 0, 0),
                         glm::vec3(0, 1, 0));
      ...
      view = glm::lookAt(glm::vec3(0, 3, 0), glm::vec3(0, 0, 0),
                         glm::vec3(0, 0, 1));
    ...
    // Atualização da matriz de view de acordo com as mudanças que ela sofreu
    // via input de mouse e/ou teclado
    view = camera.getViewMatrix();
    glUniformMatrix4fv(glGetUniformLocation(shaderID, "view"), 1, GL_FALSE,
                       glm::value_ptr(view));

    // Passa a posicao da camera para o shader
    glUniform3f(glGetUniformLocation(shaderID, "cameraPos"), camera.position.x,
...
int loadSimpleOBJ(string filePATH, int& nVertices) {
  ...
  nVertices = vBuffer.size() / 9;  // x, y, z, r, g, b,nx,ny, nz (valores
                                   // atualmente armazenados por vértice)
